...
def increment_number(number, level, base_numbers):
    """
    ...
    for paragraph in root.findall(".//w:p", namespaces=ns):
        ...
        if level is not None and level <= 4:
            ...
            if base_numbers[level] is None:
                # 初回の番号として設定し、それ以降はこの番号を基準にインクリメント
                base_numbers[level] = number
            else:
                # 上位レベルの番号を引き継ぎつつ、レベルごとにインクリメント
                if number[-1] <= base_numbers[level][-1]:
                    number = increment_number(number, level, base_numbers)
                else:
                    number[level - 1] += 1  # 正しい連番にする
