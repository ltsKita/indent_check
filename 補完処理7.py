    ...
    for run in runs:
        ...
        for level, (pattern, open_bracket, close_bracket) in bracket_patterns.items():
            if re.match(pattern, original_text):
                ...
                if re.match(r"[ 　]{2,}", after_item_number) or re.match(r"^[　]+", after_item_number):  # 余分なスペースの修正
                    after_item_number = re.sub(r"[ 　]+", " ", after_item_number).strip()  # 余分なスペースを半角スペース1つに
                    new_text = f"{item_number} {after_item_number}"

                    if new_text != original_text:  # テキストが変更された場合のみ実行
                        w_t.text = new_text
                        log_entry = f"スペース修正: '{original_text}' -> '{new_text}'"
                        log.append(log_entry)
                        highlight_text(run)  # ハイライト適用

                # 項目番号の後ろにスペースが1つもない場合には、半角スペースを1つ追加
                elif not re.match(r"^ ", after_item_number):  # スペースがない場合
                    new_text = f"{item_number} {after_item_number}"
