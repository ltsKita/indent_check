...
def process_brackets(paragraph, log):
    """レベル5,7,8,9のカッコ補完およびスペース処理"""
    runs = paragraph.findall(".//w:r", namespaces=ns)
    ...
    for run in runs:
        ...
        for level, (pattern, open_bracket, close_bracket) in bracket_patterns.items():
            if level in [5, 7, 8, 9]:
                ...
                if match:
                    ...
                    # 半角スペースが2つ以上、または全角スペースがある場合
                    if re.match(r"[ 　]{2,}", after_item_number):
                        ...
                        if new_text != original_text:
                            w_t.text = new_text
                            log_entry = f"レベル{level}: スペース修正: '{original_text}' -> '{new_text}'"
                            log.append(log_entry)
                            highlight_text(run)
                    else:
                        # 半角スペースが1つの場合は何もしない
        ...
        # カッコ補完が終わった後でスペース処理を行う
        if re.match(r"^\([a-zA-Z0-9]+\)\s*[ 　]+", original_text):
            new_text = re.sub(r"[ 　]+", " ", original_text)
            if new_text != original_text:
                w_t.text = new_text
                log_entry = f"カッコ補完後スペース修正: '{original_text}' -> '{new_text}'"
                log.append(log_entry)
                highlight_text(run)
...
def process_brackets_in_xml(xml_content):
    ...
    for paragraph in tree.findall(".//w:p", namespaces=ns):
        process_numbering(paragraph, log)  # レベル1〜4の処理
        process_brackets(paragraph, log)   # レベル5,7,8,9のカッコ補完とスペース処理
