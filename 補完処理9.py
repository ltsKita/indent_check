...
def process_fullwidth_alphabet_period(paragraph, log):
    ...
    for run in runs:
        ...
        # 全角小文字アルファベットの後にピリオドが続くパターンを検出
        match = re.match(r"([ａ-ｚ])(\.|．)(.*)", original_text)
        if match:
            letter = match.group(1)  # 全角小文字アルファベット
            period = match.group(2)  # ピリオド（半角または全角）
            after_period = match.group(3)  # ピリオドの後の文字列

            # ピリオドの後のスペースを削除
            after_period = re.sub(r"^\s+", "", after_period)

            # 半角ピリオドを全角ピリオドに変換
            if period == ".":
                period = "．"  # 全角に変換
                new_text = f"{letter}{period}{after_period}"
            ...
            # ピリオドの後にスペースがあれば削除してハイライトを適用
            else:
                new_text = f"{letter}{period}{after_period}"
                if new_text != original_text:
                    w_t.text = new_text
                    log_entry = f"スペース削除: '{original_text}' -> '{new_text}'"
                    log.append(log_entry)
                    highlight_text(run)
